            ...
            <div class="border-box mb-4" id="search-results">
                @if (session()->get('location_selected_id') != '')
                    ...
                    @if ($InventoryData['DataType'] == 'MasterData')
                        <form action="{{ route('add-kitchen-inventory-by-admin') }}" id="frm_register_add"
                            method="POST">
                            @csrf
                            ...
                            @else
                                <div class="border-box mb-4" id="search-results">
                                    <!-- Header Title -->
                                    <div class="grid-header text-center">
                                        <h6 class="m-0 text-white">No List Found For Kitchen Inventory. Please Contact
                                            To Super Admin</h6>
                                    </div>
                                </div>
                            @endif
                    ...
                    @elseif($InventoryData['DataType'] == 'LocationWiseData')
                        <form action="{{ route('update-kitchen-inventory-by-admin') }}" id="frm_register_edit"
                            method="POST">
                            @csrf
                            @if (!empty($InventoryData['data_location_wise_inventory']) && count($InventoryData['data_location_wise_inventory']) > 0)
                                @foreach ($InventoryData['data_location_wise_inventory'] as $category => $items)
        ...
        $("#locationForm").submit(function(e) {
            ...
            $(".qty-input").each(function() {
                ...
                if (value === "" || isNaN(value) || parseFloat(value) <= 0) {
                    ...
                    $(this).after(
                        '<span class="error text-danger">Please enter a valid quantity.</span>'
                        );
                } else {
